function Invoke-Script {
    ...
    $ScriptPath = $true #DEBUG

    if ($usePwsh) {
        if (-not $global:PowerShellSession) {
            $configurationName = 'PowerShell.7'
            write-host "Create new PowerShell session"
            $global:PowerShellSession = New-PSSession -ContainerId $containerId -RunAsAdministrator -ErrorAction SilentlyContinue -ConfigurationName $configurationName
